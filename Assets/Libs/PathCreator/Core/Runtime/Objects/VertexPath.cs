...
namespace PathCreation
{
    ...
    public class VertexPath
    {
        ...
        public Vector3 GetPointAtDistance(float dst, Action OnDone)
        {
            float t = dst / length;
            return GetPointAtTime(t, OnDone);
        }
        ...
        public Vector3 GetPointAtTime(float t, Action OnDonePath)
        {
            var data = CalculatePercentOnPathDataWithCallBack(t, OnDonePath);
            return Vector3.Lerp(GetPoint(data.previousIndex), GetPoint(data.nextIndex), data.percentBetweenIndices);
        }
        ...
        TimeOnPathData CalculatePercentOnPathDataWithCallBack(float t, Action OnDone)
        {
            // Constrain t based on the end of path instruction
